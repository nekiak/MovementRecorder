  ...
  public void draw(long vg, int x, int y, InputHandler inputHandler) {
    ...
    if (!recordingDir.exists() || !recordingDir.isDirectory()) {
      NanoVGHelper.INSTANCE.drawText(
          vg, "No recordings found", iX + 338, iY + 348, Color.WHITE.getRGB(), 36f, Fonts.REGULAR);
      return;
    }
    ...
    for (File file : recordingFiles) {
      if (file.isFile() && file.getName().endsWith(".movement")) {
        NanoVGHelper.INSTANCE.drawRoundedRect(
            vg, iX, iY, 1008, 50, new Color(50, 50, 50, 120).getRGB(), 12);
        NanoVGHelper.INSTANCE.drawText(
            vg,
            file.getName().replace(".movement", ""),
            iX + 12,
            iY + 24,
            Color.WHITE.getRGB(),
            18f,
            Fonts.REGULAR);
        BasicButton deleteButton =
            new BasicButton(
                120,
                BasicButton.SIZE_36,
                "Delete",
